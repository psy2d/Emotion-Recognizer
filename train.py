"""
Title: CNN for training the emotion recognizer to recognize 3 basic emotions (Happy / Sad / Neutral)
Author: Sayali Deshpande

Code for demonstrating the CNN used for training the emotion recognizer.
Requires input data in pickle format.
...
PICKLE_FILE = 'pickles/emotion.pickle'          # pickle file in which the preprocessed datasets are stored
MODEL_CKPT="checkpoints/my_model"               # checkpoint file
MODEL_NAME='emotion_classifier_temp.pb'              # file which contains the frozen graph
EXPORT_DIR="./pb"                               # directory which stores the frozen graph files

NUM_CLASSES=3                                   # 3 classes: Happy, Sad, Neutral

IMAGE_SIZE = 48                                 # square images of size IMAGE_SIZE x IMAGE_SIZE
NUM_CHANNELS = 1                                # gray-scale images
...
BETA=0.0009                                    # regularization
DROPOUT_RATE=0.9                               # drop-out

NUM_STEPS = 20001                              # number of training steps
...
with graph.as_default():
    ...
    layer2_weights = tf.get_variable("layer2_weights",shape=[PATCH_SIZE2, PATCH_SIZE2, DEPTH1, DEPTH2], initializer=tf.contrib.layers.xavier_initializer())
    layer2_biases = tf.Variable(tf.constant(1.0, shape=[DEPTH2]))
    layer3_weights =tf.get_variable("layer3_weights",shape=[PATCH_SIZE3, PATCH_SIZE3, DEPTH2, DEPTH3], initializer=tf.contrib.layers.xavier_initializer())
    layer3_biases = tf.Variable(tf.constant(1.0, shape=[DEPTH3]))
    layer4_weights =tf.get_variable("layer4_weights",shape=[PATCH_SIZE4, PATCH_SIZE4, DEPTH3, DEPTH3], initializer=tf.contrib.layers.xavier_initializer())
    layer4_biases = tf.Variable(tf.constant(1.0, shape=[DEPTH3]))
    layer5_weights =tf.get_variable("layer5_weights",shape=[PATCH_SIZE4, PATCH_SIZE4, DEPTH3, DEPTH3], initializer=tf.contrib.layers.xavier_initializer())
    layer5_biases = tf.Variable(tf.constant(1.0, shape=[DEPTH3]))
    layer6_weights = tf.get_variable("layer6_weights",shape=[IMAGE_SIZE // 8 * IMAGE_SIZE // 8 * DEPTH3, NUM_HIDDEN1], initializer=tf.contrib.layers.xavier_initializer())
    layer6_biases = tf.Variable(tf.constant(1.0, shape=[NUM_HIDDEN1]))
    layer7_weights = tf.get_variable("layer7_weights",shape=[NUM_HIDDEN1, NUM_HIDDEN2], initializer=tf.contrib.layers.xavier_initializer())
    layer7_biases = tf.Variable(tf.constant(1.0, shape=[NUM_HIDDEN2]))
    layer8_weights = tf.get_variable("layer8_weights",shape=[NUM_HIDDEN2, NUM_HIDDEN3], initializer=tf.contrib.layers.xavier_initializer())
    layer8_biases = tf.Variable(tf.constant(1.0, shape=[NUM_HIDDEN3]))
    layer9_weights = tf.get_variable("layer9_weights",shape=[NUM_HIDDEN3, NUM_CLASSES], initializer=tf.contrib.layers.xavier_initializer())
    ...
    def model(data,keep_prob):
        ...
        hidden4 = tf.nn.dropout(tf.nn.relu(conv4 + layer4_biases),keep_prob)
        hidden4=maxpool2d(hidden4,2)
        conv5= tf.nn.conv2d(hidden4, layer5_weights, [1, 1, 1, 1], padding='SAME')
        hidden5 = tf.nn.dropout(tf.nn.relu(conv5 + layer5_biases),keep_prob)
        hidden5=maxpool2d(hidden5,2,3)
        ...
        fc2 = tf.nn.dropout(tf.nn.relu(tf.matmul(fc, layer7_weights) + layer7_biases),keep_prob)
        fc3 = tf.nn.dropout(tf.nn.relu(tf.matmul(fc2, layer8_weights) + layer8_biases),keep_prob)
    ...
    regularizers=tf.nn.l2_loss(layer1_weights)+tf.nn.l2_loss(layer2_weights)+tf.nn.l2_loss(layer3_weights)+tf.nn.l2_loss(layer4_weights)+tf.nn.l2_loss(layer5_weights)+tf.nn.l2_loss(layer6_weights)+tf.nn.l2_loss(layer7_weights)
...
g = tf.Graph()
with g.as_default():
    x_input = tf.placeholder(tf.float32, shape=[None,IMAGE_SIZE,IMAGE_SIZE,NUM_CHANNELS], name="input")

    WC1 = tf.constant(WC1, name="WC1")
    BC1 = tf.constant(BC1, name="BC1")
    CONV1 = conv2d(x_input, WC1, BC1)
    MAXPOOL1 = maxpool2d(CONV1,2,3)
